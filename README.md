...
图标出现在菜单栏右上角 —— 一只 Clawd 像素小螃蟹，平时原地休息，有 LLM 请求
在途时会爬起来并显示计时（见「菜单栏小机器人」）。
...
## ZCode 用量旁路（统计 ZCode 自身的调用）

ZCode 客户端（内置渠道）的模型调用**不经过本代理**，但它的每次调用都记在自己的
账本里：`~/.zcode/cli/db/db.sqlite` 的 `model_usage` 表。`zcode_reader.py` 定期
...
菜单栏图标是 Clawd 像素小螃蟹（帧数据照抄自
[claude-status-bar](https://github.com/m1ckc3s/claude-status-bar)，MIT），带开关：

- **开启**（默认）—— 有 LLM 请求在途时爬行（20 帧 / 12.5 FPS，与源 GIF 同节奏），
  右侧跟一块 `1m 32s` 的回合计时
- **关闭** —— 回到原版的黄色 ⚡ 图标：无动画、无计时、无「进行中」横幅，
  连 0.5 秒的状态轮询都停掉。开关在**状态栏图标右键菜单**里（「Clawd 小螃蟹」
  勾选项），存 UserDefaults（`menuBarCrabEnabled`），重启保留

数据流很短：代理在「请求开始 / 流式 chunk / 请求结束」三个点写
`proxy/ai_status.json`，菜单栏应用 0.5 秒轮询一次。用文件而不是端口/socket，
是因为 app 侧本来就有按目录读文件的一整套基建，且这个小文件不到 1KB、
忙碌期间最多每秒写一次，轮询成本可以忽略。

### 「回合」的语义

代理看不到没有网络流量的阶段（工具在本地执行、等待权限确认），只能用
**时间窗口去猜**「这一轮结束了没」：

| 现象                   | 判定                                                                     |
| ---------------------- | ------------------------------------------------------------------------ |
| 螃蟹爬行 + 计时         | 有请求在途；或最后一次活动过去没超过 15 秒（回合尾巴）                    |
| 间隙 < 15 秒的下一个请求 | 算**同一回合**，计时沿用原起点不归零 —— 工具执行完接着问的场景计时是连续的 |
| 15 秒无任何活动         | 回合结束，螃蟹停住                                                       |
| 文件过旧（> 20 秒）且仍标着在途 | 代理失联/崩溃，**强制判空闲** —— 心跳兜底（忙碌期 5 秒一跳）正常到不了这个阈值 |

15 秒这个值是代理侧 `LIVE_STATUS_QUIET_SECONDS` 与 app 侧
`LiveStatusSemantics.quietSeconds` 的一对常量，**改一边必须改另一边**
（app 侧没有 selftest，靠这条注释互钉）。

已知盲区：工具一跑几分钟、或者权限确认停留很久，螃蟹会中途歇下来 ——
这是纯代理驱动的固有边界。claude-status-bar 本体靠往 `~/.claude/settings.json`
注入 Claude Code hooks 才能看到这些阶段；TokenScope 选了零系统配置改动的路线，
换来的是对所有走代理的客户端（Claude Code、Cursor、任意 curl）通用。
探活请求（无 model 的 GET 等）不触发动画，与「无 model 不落盘」同一条判据。
...
```json
{
  "pid": 42314,
  "in_flight": 1,
  "turn_start": 1787971826.93,
  "last_activity": 1787971830.11,
  "updated_at": 1787971830.11,
  "requests": [
    {"provider": "deepseek", "model": "deepseek-chat", "stream": true, "started": 1787971826.93}
  ]
}
```

- 时间戳一律是 **epoch 秒**（`time.time()` / `Date().timeIntervalSince1970`），
  不用 jsonl 统计里的字符串格式 —— 两个进程要对齐的是「现在」，字符串时间
  徒增解析与时区两个出错点
- **写入方只有代理**（原子写：tmp + `os.replace`），应用只读 —— 与统计 jsonl
  同一条纪律；原子写保证轮询方永远读不到半行 JSON
- 空闲期间代理**不写**这个文件，回合结束时落最后一次 `in_flight: 0`；
  文件不存在（代理没在跑）app 按空闲处理，螃蟹睡觉

### app 侧结构

`RobotMonitor` 是进程级单例（挂在 App 层），弹窗关着也照常轮询 ——
与 `StatsViewModel`（30 秒、只在弹窗打开时跑）刻意分成两个节奏、两个数据源。
弹窗 header 下有一条**仅忙碌时出现**的横幅：`● 进行中 · N 个请求 · 1m32s`。

菜单栏的承载从 `MenuBarExtra` 换成了 `NSStatusItem + NSPopover`：逐帧换图、
实时计时文本这些需求走 SwiftUI label 重求值不可控，直接下探 AppKit
（与 `WindowAppearance` 同一个取舍先例）。左键弹统计面板（内容仍是原来的
`ContentView`），右键是「打开统计面板 / 退出」菜单。
